var rmrp = function(req,res){
  ...
  if(database){
      ...
      if(results.length>0){
        ...
        database.TkModel.find({_id:_id},function(err,results){
          ...
          if(results.length>0){
            ...
            database.PostModel.find({_id:p_id},function(err,results){
              if(err){
              ...
              if(results.length>0){
                ...
                if(ci!=-1){
                  database.PostModel.where({_id:p_id}).update({cn:cn},function(err){
                    if(err){
                    ...
                    database.CommentModel.where({_id:c_id}).update({created_time:1},function(err){
                      if(err){
    ...
    console.log('rmrp: no database');
    output.status=410;
    res.send(output);  
  }    
}
module.exports.rmrp = rmrp;