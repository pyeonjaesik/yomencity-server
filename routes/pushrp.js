var pushrp = function(req,res){
  ...
  var al2=[];
  cn--;
  var al3={type:'2',id:id1,name:name,post_id:post_id,img:img,cn:cn,ct:ct,d:1};
  if(database){
    database.UserModel.find({_id:_id1},function(err,results){
      ...
      if(results.length>0){
                ...
                if(results.length>0){
                  ...
                  database.AlModel.find({_id:t_id},function(err,results){
                      ...
                      if(results.length>0){
                        ...
                        al2.unshift({type:'2',id:id1,name:name,post_id:post_id,cn:cn,ct:ct,d:1});
        ...
        console.log('UserModel.find length==0 --> err');
        output.status=502;
        res.send(output);
        return;  
      }    
    ...
    console.log('pushrp: no database');
    output.status=410;
    res.send(output);  
  }    
};
module.exports.pushrp = pushrp;